function [total_error_per_point, indv_error_per_point, trained_parameters, total_parameters] ...
    = ten_fold_cross_validation(host_site,sg_RNA,...
    ...
    data_permutation = randperm(length(binding_rate));
    % generate folds
    batches = reshape(data_permutation, 10, floor(length(binding_rate)/10));
    ...
    for data_point = 1:10
        ...
        current_host_site(batches(data_point,:),:) = [];
        current_sg_RNA = sg_RNA;
        current_sg_RNA(batches(data_point,:),:) = [];
        current_binding_rate = binding_rate;
        current_binding_rate(batches(data_point,:),:) = [];
        % train model
        use_old = 0;
